#!/usr/bin/env bash
# signin-workbuddy.sh — WorkBuddy/CodeBuddy 批量签到脚本，签到后通过 ntfy 推送通知
set -e
cd "$(dirname "$0")"

# ntfy 推送地址：运行前请自行 export NTFY_URL='https://ntfy.sh/你的主题'
# 注意：不要将真实 ntfy 主题硬编码进仓库。GitHub Actions 通过 secrets.WORKBUDDY_NTFY_URL 注入。
NTFY_URL="${NTFY_URL:-}"
if [ -z "$NTFY_URL" ]; then
  echo "⚠️ 未设置 NTFY_URL，跳过 ntfy 推送。本地运行可先: export NTFY_URL='https://ntfy.sh/你的主题'"
fi

go build -o workbuddy_bin ./cmd/workbuddy

# 执行签到并捕获输出
SIGNIN_OUTPUT=$(./workbuddy_bin "${1:-auths}" 2>&1)
SIGNIN_EXIT=$?
...
if [ "$SIGNIN_EXIT" -eq 0 ]; then
    if [ "$OK" -gt 0 ] 2>/dev/null; then
        TITLE="✅ WorkBuddy 签到成功"
        TAG="white_check_mark"
    elif [ "$FAIL" -gt 0 ] 2>/dev/null; then
        TITLE="⚠️ WorkBuddy 签到异常"
        TAG="warning"
    else
        TITLE="📌 WorkBuddy 已签到"
        TAG="pushpin"
